...
class Model:
	...
	def combine(self, prev_extractor_output, curr_extractor_output):
		with tf.variable_scope('combiner', reuse=self.reuse_combiner):
			context_vector = prev_extractor_output + curr_extractor_output
	...
	def train_on_batch(self, batch, session, tf_extractor_input, tf_prev_extractor_output, tf_decoder_input, tf_extractor_output, tf_target_output, tf_mask, tf_train_op, tf_loss, tf_precision):
		...
		for j in range(num_parts):
			#for i in range(num_samples):
			extractor_input = []
			prev_extractor_output = np.zeros([num_samples, EXTRACTOR_OUTPUT_DIM], dtype=np.float32)
			decoder_input = []
			target_output = []
			for i in range(num_samples):
				extractor_input.append(images[i][:, j*STRIDE: j*STRIDE + CUT_WIDTH])
				decoder_input.append(sub_texts[j][i][:-1])
				target_output.append(sub_texts[j][i][1:])
			extractor_input = np.float32(extractor_input)
			...
			_, loss_val, precision_val, prev_extractor_output = session.run(
				[tf_train_op,
				tf_loss,
				tf_precision,
				tf_extractor_output],
				feed_dict={
					tf_extractor_input: extractor_input,
					tf_prev_extractor_output: prev_extractor_output,
	...
	def train(self, num_epochs=1000, batch_size=10, image_folder='./image/', dataset_file='./dataset.txt', model_path='./model/model', resume=False):
		...
		FX = self.extract(X, training=True)
		
		prev_FX = tf.placeholder(tf.float32, shape=[None, EXTRACTOR_OUTPUT_DIM])
		prev_Y = tf.placeholder(tf.int32, shape=[None])
		C = self.combine(prev_FX, FX)
		...
		for i in range(num_epochs):
			...
			for j in range(0, num_data, batch_size):
				...
				loss_val, precision_val = self.train_on_batch(batch, session, X, prev_FX, prev_Y, FX, Y, mask, train_op, loss, precision)
