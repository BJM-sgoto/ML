...
class Model:
	...
	def train_on_batch(self, batch, session, tf_encoder_input1, tf_encoder_input2, tf_decoder_input, tf_target_output, tf_mask, tf_train_op, tf_loss, tf_precision, tf_decoder_output):
		...
		print(input_texts)		
		print(target_texts)
		print(np.argmax(pred_text, axis=2))
		exit()
	...
	def test(self, image_folder='./dataset/', dataset_file='./dataset.txt', model_path='./model/model'):
		...
		image = np.float32(cv2.imread(test_dataset[1][0]))
		image, _ = self.pad_image(image, CUT_HEIGHT, CUT_WIDTH + (NUM_FEATURES - 1) * STRIDE)
		images = np.float32([image])
		batch_size = 1
		texts = ['' for j in range(batch_size)]
		for i in range(NUM_FEATURES - 1):
			sub_texts = ['' for j in range(batch_size)]
			images1 = images[:,:, i*STRIDE:i*STRIDE+CUT_WIDTH]
			images2 = images[:,:, (i+1)*STRIDE:(i+1)*STRIDE+CUT_WIDTH]
			cv2.imwrite('images1_{:06d}.png'.format(i),images1[0])
			cv2.imwrite('images2_{:06d}.png'.format(i),images2[0])
			context = session.run(pred_C, feed_dict={X1: images1, X2: images2})
			prev_texts = np.float32(np.ones([batch_size, 1]) * self.dictionary[CHAR_START])
			ends = np.int32(np.zeros(batch_size))
			
			while np.sum(ends) < batch_size:
				prev_texts, context = session.run(
					[pred_Y, decoder_S],
					feed_dict={
						C: context,
						prev_Y: prev_texts})
				prev_texts = np.argmax(prev_texts, axis=2)
				for j in range(batch_size):
					if ends[j]==False:
						if prev_texts[j,0]==self.dictionary[CHAR_END]:
							ends[j] = True
						else:
							sub_texts[j] += reversed_dictionary[prev_texts[j,0]]
