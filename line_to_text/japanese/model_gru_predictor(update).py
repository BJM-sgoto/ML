...
class Model:
	...
	def decode(self, context_vector, decoder_text_input):
		with tf.variable_scope('decoder', reuse=self.reuse_decoder):
			...
			decoder_dictionary = tf.get_variable('decoder_dictionary', shape=[len(self.dictionary), 256], dtype=tf.float32)
	...
	def train(self, num_epochs=1000, batch_size=10, image_folder='./image/', dataset_file='./dataset.txt', model_path='./model/model', resume=False):
		...
		loss = tf.reduce_sum(tf.square(labels - pred_Y), axis=2)
		loss = loss * mask
	...
	################### This code is to expand dictionary ###################
	UPDATE_VARIALE_NAMES = [
	'decoder/decoder_dictionary:0',
	'decoder/dense/kernel:0',
	'decoder/dense/bias:0',
	'decoder/decoder_dictionary/Adam:0',
	'decoder/decoder_dictionary/Adam_1:0',
	'decoder/dense/kernel/Adam:0',
	'decoder/dense/kernel/Adam_1:0',
	'decoder/dense/bias/Adam:0',
	'decoder/dense/bias/Adam_1:0']
	def save_changed_variables(self, model_path='./model/model'):
		X = tf.placeholder(tf.float32, shape=[None, CUT_HEIGHT, CUT_WIDTH, 3])
		FX = self.encode(X, training=True)
		...
		saver = tf.train.Saver()
		saver.restore(session, model_path)
		update_variables = [variable for variable in tf.global_variables() if variable.name in Model.UPDATE_VARIALE_NAMES]
		for i, update_variable in enumerate(update_variables):
			val = session.run(update_variable)
			np.save('{:06d}.npy'.format(i), val)
			print('Save', '{:06d}.npy'.format(i))
		session.close()
	
	# after change length of dictionary call this function
	def update_changed_variables(self, model_path='./model/model'):
		X = tf.placeholder(tf.float32, shape=[None, CUT_HEIGHT, CUT_WIDTH, 3])
		FX = self.encode(X, training=True)
...
model = Model()
#model.train(num_epochs=60, batch_size=BATCH_SIZE, image_folder='./train_dataset/', dataset_file='./train_data.txt', model_path='./model_gru_predictor/model', resume=True)
#model.test(image_folder='./test_dataset/', dataset_file='./test_img_text.txt',  model_path='./model_gru_predictor/model')
model.save_changed_variables(model_path='./model_gru_predictor/model')


''' Step 1	
decoder_dictionary = tf.trainable_variables('decoder/decoder_dictionary')[0]
dictionary_val = session.run(decoder_dictionary)
np.save('data.npy', dictionary_val)
'''
...
session = tf.Session()
variables = tf.global_variables()
variables = [variable for variable in variables if variable.name!='decoder/decoder_dictionary']
...
dictionary_val = np.zeros([3034, 256], np.float32) # 3034 common kanji
old_dictonary_val = np.load('data.npy')
dictionary_val[:old_dictonary_val.shape[0], :] = old_dictonary_val
decoder_dictionary = tf.trainable_variables('decoder/decoder_dictionary')[0]
assign_op = tf.assign(decoder_dictionary, dictionary_val)
