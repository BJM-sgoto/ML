...
class H_Bird:
	...
	def step(self, delta_frames=1):
		delta_time = frame_to_msec(delta_frames)
		if self.msec_to_climb>0:
			frac_climb_done = 1 -  self.msec_to_climb/H_Bird.CLIMB_DURATION
...
class QuickEnvironment:
	...
	def reset(self, state_type=STATE_TYPE_NUMBERS):
		self.clock_tick = 0
		self.frame_count = 0
		self.bird.y = np.random.randint(low=WIN_HEIGHT/4, high=3*WIN_HEIGHT/4-H_Bird.HEIGHT)
	...
	def step(self, action=ACTION_DO_NOTHING):
		# increase clock tick
	...
	def get_state(self, state_type=STATE_TYPE_NUMBERS):
		if state_type==STATE_TYPE_NUMBERS:
			return [self.bird.y/191 - 1, self.bird.speed/H_Bird.SINK_SPEED, self.get_next_pipe_pos()/223 - 1]
...
count = 0
while not done:
	reward, done = env.step(action=np.random.randint(2))
	img = env.screenshot()
