...
class PolicyValueNet:	
	...
	def train_step(self, state_batch, mcts_probs, actions, winner_batch, lr):
		next_state_batch = -(state_batch + actions)
		value_batch = self.session.run(self.evaluation, feed_dict={self.input_states: state_batch})
		next_value_batch = -self.session.run(self.evaluation, feed_dict={self.input_states: next_state_batch})
		target_value = value_batch + DECAY * (next_value_batch - value_batch)
		target_value[-1,0] = 1
