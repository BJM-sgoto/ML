...
class Game:
	...
	def start_self_play(self, player, is_shown=0, temp=1e-3):
		...
		p1, p2 = self.board.players
		states, mcts_probs, actions, current_players = [], [], [], []
		while True:
			move, move_probs = player.get_action(self.board, temp=temp, return_prob=1)
			states.append(self.board.current_state())
			mcts_probs.append(move_probs)
			action_board = np.zeros(self.board.width * self.board.height, dtype=np.float32)
			action_board[move] = 1.0
			action_board = action_board.reshape(self.board.height, self.board.width)
			...
			if end:
				...
				if is_shown:
					if winner != P_E:
						...
						print('game end.Tie')
				return winner, zip(states, mcts_probs, actions, winners_z)