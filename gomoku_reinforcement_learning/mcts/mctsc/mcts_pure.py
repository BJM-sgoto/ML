...
class TreeNode:
	...
	def update(self, leaf_value, virtual_loss=0.05):
		self._Q += leaf_value + virtual_loss
		
	def update_recursive(self, leaf_value, virtual_loss=0.05):
		if self._parent:
			self._parent.update_recursive(-leaf_value, virtual_loss)
			self.update(leaf_value, virtual_loss)
		
	def is_leaf(self):
		return len(self._children)==0
		
	def get_value(self):
		return (self._Q + C_PUCT * self._P * math.sqrt(self._parent._n_visits))/(1 + self._n_visits)
...
class MCTS:	
	...
	def _playout(self, board):
		...
		for i in range(NUM_INSTANCE):
			...
			node.update_recursive(-leaf_value, virtual_loss=VIRTUAL_LOSS)
