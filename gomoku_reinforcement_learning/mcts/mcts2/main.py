...
from constants import BOARD_WIDTH, BOARD_HEIGHT, N_IN_ROW, LEARN_RATE, LR_MULTIPLIER, TEMPERATURE, NUM_PLAYOUT, C_PUCT, BUFFER_SIZE, BATCH_SIZE, KL_TARGET, CHECK_FREQ, GAME_BATCH_NUM, BEST_WIN_RATIO, NUM_PURE_MCTS_PLAYOUT, PLAY_BATCH_SIZE, EPOCHS, RANDOM_SEED
...
class TrainPipeline:
	...
	def get_equi_data(self, play_data):
		...
		for state, mcts_prob, winner in play_data:
			for i in [1,2,3,4]:
				equi_state = np.float32([np.rot90(s, i) for s in state])
	...
	def policy_evaluate(self, n_games=10):
		...
		for i in range(n_games):
			winner = self.game.start_play(
				...
				is_shown=0)
			win_count[winner]+=1
		win_ratio = 1.0*(win_count[1] + 0.5*win_count[-1])/n_games
		print('num_playouts: {}, win: {}, loss: {}, tie: {}'.format(self.pure_mcts_playout_num, win_count[1], win_count[2], win_count[-1]))
		return win_ratio
		...
		try:
			for i in range(self.game_batch_num):
				...
				if (i+1)%self.check_freg==0:
					...
					self.policy_value_net.save_model('./model/current_model')
					if win_ratio>self.best_win_ratio:
						self.best_win_ratio = win_ratio
						print('New best policy!!!')
						self.policy_value_net.save_model('./model/best_model')
						if self.best_win_ratio>0.99 and self.pure_mcts_playout_num<5000:
