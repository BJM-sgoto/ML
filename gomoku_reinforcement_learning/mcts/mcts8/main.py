...
from constants import BOARD_WIDTH, BOARD_HEIGHT, N_IN_ROW, LEARN_RATE, LR_MULTIPLIER, TEMPERATURE, NUM_PLAYOUT, BUFFER_SIZE, BATCH_SIZE, KL_TARGET, CHECK_FREQ, GAME_BATCH_NUM, BEST_WIN_RATIO, NUM_PURE_MCTS_PLAYOUT, PLAY_BATCH_SIZE, EPOCHS, RANDOM_SEED, P_X, P_O, P_E
...
class TrainPipeline:
	def __init__(self, resume=False):
		...
		if resume:
			...
			self.policy_value_net = PolicyValueNet(self.board_width, self.board_height, model_file=BEST_MODEL_PATH)
		else:
	...
	def read_cache(self, cache_file):
		...
		self.pure_mcts_playout_num = int(f.readline().strip())
		f.close()
...
if __name__ == '__main__':
	training_pipleline = TrainPipeline(resume=False)
