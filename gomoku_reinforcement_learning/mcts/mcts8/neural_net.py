...
import numpy as np
from constants import RANDOM_SEED, P_E
...
class PolicyValueNet:	
	...
	def policy_value_fn(self, board):
		log_act_probs, value = self.session.run(
			[self.policy, self.evaluation],
			feed_dict={self.input_states:np.float32([board.current_state()])})
		value = value[0]
		log_act_probs = log_act_probs[0]
		act_probs = np.exp(log_act_probs - np.max(log_act_probs))
