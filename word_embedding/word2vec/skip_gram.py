...
class Model:
	...
	def build_model(self, encoded_input):
		with tf.variable_scope("Skip_Gram"):
			encoded_vocab = tf.get_variable("encoded_vocab", dtype=tf.float32, shape=[VOCAB_SIZE, ENCODE_DIM])
			x = tf.expand_dims(encoded_input, axis=1)
			x = tf.gather_nd(encoded_vocab, x)
			y = tf.layers.dense(x, units=VOCAB_SIZE)
			y = tf.nn.softmax(y, axis=-1)
		return y, encoded_vocab

	def sample(self, dataset, n_samples, n_gram=4):
		...
		for i in range(n_samples):
			...
			min_neighbor_id = max(0, word_id - n_gram)
			max_neighbor_id = min(sentence_len, word_id + n_gram + 1)
			neighbor_ids = list(range(min_neighbor_id, word_id)) + list(range(word_id+1, max_neighbor_id))
			neighbor_id = np.random.choice(neighbor_ids)
			inputs.append(sentence[word_id])
			outputs.append(sentence[neighbor_id])
	...
	def train(self, train_file, test_file, processed_file, vocab_file, first_time=True, n_steps=50000, batch_size=512,model_path='./model/cbow', resume=False):
		...
		PY, encoded_vocab = self.build_model(X)
		loss = tf.reduce_mean(tf.reduce_sum(tf.square(PY - tf.one_hot(X, depth=VOCAB_SIZE, dtype=tf.float32)), axis=1))
		...
		for i in range(n_steps):
			inputs, outputs = self.sample(dataset, batch_size)
			loss_val, _ = session.run([loss, train_op], feed_dict={X: inputs, Y: inputs})
