...
class Model:
	...
	def train_batch(self, batch, session, real_inputs, fake_input, generator_cost, discriminator_cost, generator_train_op, discriminator_train_op):
		...
		for image_file in batch:
			...
			image_3 = cv2.resize(image_4, (int(IMG_HEIGHT/2), int(IMG_HEIGHT/2)))
			image_2 = cv2.resize(image_3, (int(IMG_HEIGHT/4), int(IMG_HEIGHT/4)))
			image_1 = cv2.resize(image_2, (int(IMG_HEIGHT/8), int(IMG_HEIGHT/8)))
			rx1.append(image_1)
			rx2.append(image_2)
			rx3.append(image_3)
			rx4.append(image_4)
