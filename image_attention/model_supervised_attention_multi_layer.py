...
class Model:
	...
	def estimate(self, local_features, global_feature):
		with tf.variable_scope('estimator', reuse=self.reuse_estimator):
			...
			for local_feature in local_features:
				...
				local_feature_height = local_feature_shape[1].value
				local_feature_width = local_feature_shape[2].value
				if max_feature_height < local_feature_height:
					max_feature_height = local_feature_height
				if max_feature_width < local_feature_width:
					max_feature_width = local_feature_width
			...
			for local_feature in local_features:
				...
				local_feature_height = local_feature_shape[1].value
				local_feature_width = local_feature_shape[2].value
				repeat_y = int(max_feature_height / local_feature_height)
				repeat_x = int(max_feature_width / local_feature_width)
				local_feature = tf.expand_dims(local_feature, axis=2)
				local_feature = tf.tile(local_feature, [1,1,repeat_y,1,1])
				local_feature_shape = local_feature.get_shape()
				local_feature = tf.reshape(local_feature, [-1, local_feature_shape[1].value * local_feature_shape[2].value, local_feature_shape[3].value,local_feature_shape[4].value])
				local_feature = tf.expand_dims(local_feature, axis=3)
				local_feature = tf.tile(local_feature, [1,1,1,repeat_x,1])
				local_feature_shape = local_feature.get_shape()
				local_feature = tf.reshape(local_feature, [-1, local_feature_shape[1].value, local_feature_shape[2].value*local_feature_shape[3].value,local_feature_shape[4].value])
			...
			global_feature_projection = tf.tile(global_feature_projection, [1, max_feature_height, max_feature_width, 1])
			features.append(global_feature_projection)
	...
	def compute_cost(self, compatibility_score, target_mask):
	...
	def test(self, batch_size=BATCH_SIZE, image_folder='./image/', output_folder='./output/', model_path='./model/model'):
		X = tf.placeholder(tf.float32, shape=[None, IMG_HEIGHT, IMG_WIDTH, 3])
		...
		for i in range(0, num_data, batch_size):
			...
			for j in range(i, end_i):
				...
				images.append(image)
			images = np.float32(images)
			score = session.run(A, feed_dict={X: images})
			score = score * 255
			for j in range(i, end_i):
				cv2.imwrite(output_folder + files[j], score[j-i])
		session.close()
...
model.train(
	...
	num_epoch=100,
	image_folder='./image/zebra/',
	mask_folder='./mask/zebra/',
	model_path='./model/zebra/model_supervised_attention_multi_layer',
	resume=True)
'''

model.test(
	batch_size=BATCH_SIZE, 
	image_folder='./test_image/zebra/', 
	output_folder='./output/zebra/model_supervised_attention_multi_layer/', 
	model_path='./model/zebra/model_supervised_attention_multi_layer')
