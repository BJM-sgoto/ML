...
class Model:
	...
	def up_resblock(self, x, layer_depth, use_batch_norm=True, use_spectral_norm=True, training=False, scope='up_resblock'):
		with tf.variable_scope(scope):
			x_init = x
			height = x.shape[1].value
			width = x.shape[2].value
	...
	def down_resblock(self, x, layer_depth, use_batch_norm=True, use_spectral_norm=True, training=False, scope='down_resblock'):
		with tf.variable_scope(scope):
			x_init = x
			height = x.shape[1].value
			width = x.shape[2].value
	...
	def generate(self, z, training=False):
		with tf.variable_scope('generator', reuse=self.reuse_generator):
			...
			layer_depths = [256,256,128,128,64]
			for i, layer_depth in enumerate(layer_depths):
				if i!=len(layer_depths)-1:
					x = self.up_resblock(x, layer_depth, use_batch_norm=False, use_spectral_norm=True, training=training, scope='up_resblock_' + str(i))
				else:
					x = self.up_resblock(x, layer_depth, use_batch_norm=False, use_spectral_norm=True, training=training, scope='up_resblock_' + str(i))
	...
	def discriminate(self, x, training=False):
		with tf.variable_scope('discriminator', reuse=self.reuse_discriminator):
			layer_depths = [32,64,128,64,32]
			for i, layer_depth in enumerate(layer_depths):
				if i!=len(layer_depths)-1:
					x = self.down_resblock(x, layer_depth, use_batch_norm=False, use_spectral_norm=True, training=training, scope='down_resblock_' + str(i))
				else:
					x = self.down_resblock(x, layer_depth, use_batch_norm=False, use_spectral_norm=True, training=training, scope='down_resblock_' + str(i))
	...
	def compute_cost(self, predicted_outputs):
		...
		target_outputs = tf.concat(
									[tf.random_uniform([batch_size], minval=0.900, maxval=0.999),
									tf.random_uniform([batch_size], minval=0.001, maxval=0.100)],
									axis=0)
		discriminator_cost = tf.reduce_mean(tf.square(target_outputs-predicted_outputs))
		fake_outputs = tf.random_uniform([batch_size], minval=0.900, maxval=0.999)
		generator_cost = tf.reduce_mean(tf.square(fake_outputs- predicted_outputs[batch_size: 2*batch_size]))
		return discriminator_cost,generator_cost
				
	def train_on_batch(self, session, batch, tf_random_inputs, tf_images, tf_generator_cost, tf_discriminator_cost, tf_train_generator_op, tf_train_discriminator_op, tf_predicted_outputs):
		images = []
		...
		random_inputs = np.float32(np.random.uniform(low=-1, high=1, size=[len(images), Z_DIM]))
		# train generator
		generator_loss_val, _, predicted_outputs = session.run([tf_generator_cost, tf_train_generator_op, tf_predicted_outputs],
			feed_dict={tf_random_inputs: random_inputs,
				tf_images: images})
		#print('predicted_outputs 1', predicted_outputs)
		
		# train discriminator
		discriminator_loss_val, _, predicted_outputs = session.run([tf_discriminator_cost, tf_train_discriminator_op, tf_predicted_outputs],
			feed_dict={tf_random_inputs: random_inputs,
				tf_images: images})
		#print('predicted_outputs 2', predicted_outputs)
		return generator_loss_val, discriminator_loss_val
	
	def generate_image(self, session, tf_random_inputs, tf_output_images):
		random_inputs = np.float32(np.random.uniform(low=-1, high=1, size=[20, Z_DIM]))
	...
	def train(self, num_epochs=10, batch_size=20, image_folder='./image/', model_path='./model/model', resume=False):
		...
		train_generator_op = tf.train.AdamOptimizer().minimize(generator_cost, var_list=tf.trainable_variables('generator'))
		...
		train_discriminator_op = tf.train.AdamOptimizer().minimize(discriminator_cost, var_list=tf.trainable_variables('discriminator'))
		...
		for i in range(num_epochs):
			...
			for j in range(0, num_data, batch_size):
				...
				generator_loss_val, discriminator_loss_val = self.train_on_batch(session, batch, Z, RX, generator_cost, discriminator_cost, train_generator_op, train_discriminator_op, PY)
