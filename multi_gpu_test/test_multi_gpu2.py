...
class CustomModel:
	...
	def combine_model(self, session, assign_ops):
	...
	def train(self, num_epochs=5, batch_size=40, train_folder='./train/', model_path='./multi_gpu/model', resume=False, history_path='./multi_gpu/history.txt'):
		...
		assign_ops = []
		for i in range(num_trainable_variables):
			combined_variable = (gpu0_trainable_variables[i] + gpu1_trainable_variables[i])/2
			assign_ops.append(tf.assign(gpu0_trainable_variables[i], combined_variable))
			assign_ops.append(tf.assign(gpu1_trainable_variables[i], combined_variable))
		for i in range(num_moving_variables):
			combined_variable = (gpu0_moving_variables[i] + gpu1_moving_variables[i])/2
			assign_ops.append(tf.assign(gpu0_moving_variables[i], combined_variable))
			assign_ops.append(tf.assign(gpu1_moving_variables[i], combined_variable))
		...
		for i in range(num_epochs):
			...
			for j in range(0, num_data, batch_size):
				...
				cpu_time, gpu_time = self.combine_model(session, assign_ops)
				sum_cpu_time+=cpu_time
				sum_gpu_time+=gpu_time
				count_to_save+=1
				if count_to_save>=50:
